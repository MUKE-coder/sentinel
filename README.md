...
**Rate limits and lockouts that hold across replicas.**

- **Shared counters.** Before, counters lived in each process, so behind N instances a client got N times every rate limit and N times AuthShield's failed-login budget. Set `Config.Counters: redisstore.New(client)` and every replica counts against the same numbers in Redis. It works with a single server, Redis Sentinel, or a cluster. Each decision is atomic, and requests are allowed if Redis is down.
- **`sentinel.CounterStore`** is an interface, with a `countertest` conformance suite for your own store. [`examples/multi-replica`](examples/multi-replica) runs two replicas behind Caddy with shared Redis and Postgres.
...
- **Dashboard compliance reports show data** — the Reports page read fields the API never returned, so every figure was blank. It now renders the real reports, including the provenance block and its warnings.
...
- **GORM plugin no longer panics on has-many / batch creates** (#15) — `db.Create(&order)` with a `[]Item` association handed the audit callback a slice, and primary-key extraction did a struct-only reflect access → panic → 500. Extraction is now kind-aware (slices audit a comma-joined ID list, maps skip safely), batch payloads are captured as `{"records": [...]}`, and every plugin callback is panic-guarded so auditing can never break the write it audits.
...
- **Globstar after a segment wildcard works** (#12) — `/api/apps/*/products/**` now matches the parameterised subtree and any depth below it; previously it compiled to a dead literal prefix with no warning.
